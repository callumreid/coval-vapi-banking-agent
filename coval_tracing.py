"""Coval OpenTelemetry tracing for the Vapi banking webhook.

The Vapi path is PSTN-oriented, so this module buffers spans until a Coval
simulation output ID is registered for the call. It then exports the finished
...
SERVICE = os.environ.get("COVAL_SERVICE_NAME", "coval-vapi-banking-agent")
...
def _new_trace_state(call_id: str) -> CallTraceState:
    resource = Resource.create(
        {
            ...
            "agent.provider": "vapi",
            "agent.name": "Bronchase Bank Cassidy",
    ...
    root_span.set_attribute("customer.workflow", "banking")
...
def _find_claimable_call() -> CallTraceState | None:
    candidates = [state for state in _calls.values() if not state.simulation_id and not state.exported]
    if not candidates:
        return None
...
def _parse_tool_result(result: str) -> dict[str, Any]:
    ...
    if isinstance(parsed.get("transactions"), list):
        result_count = len(parsed["transactions"])
    return {
...
def record_tool_call(
    *,
    ...
    workflow_span = _start_child_span(state, f"banking.workflow.{name}", start_ns=start_ns)
...
def finish_call(
    ...
    state.root_span.set_attribute("workflow.completed", 1 if state.tool_call_count and not state.tool_failure_count else 0)
    state.root_span.set_attribute("workflow.dependency_blocked", 1 if state.tool_failure_count else state.dependency_blocked)
